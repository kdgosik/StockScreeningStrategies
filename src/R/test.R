...
## TODO: BROKEN due to changes in screen-pattern-match.R code, argument stockdata instead of symbol

create_plot_data <- function( symbol, method = "loess", test_points = 40 ) {
  
  ## using quantmod API to get the stock data
  stockdata <- getSymbols(symbol, auto.assign = FALSE)
  ...
  out <- screen_pattern_match(symbol = symbol, method = method, test_points = test_points)
  
  test_data <- stockdata[(NROW(stockdata) - test_points) : NROW(stockdata),]
...
plot_data <- create_plot_data(symbol = "NVDA", method = "loess", test_points = 40)
